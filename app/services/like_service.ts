...
export class LikeService {
  ...
  public async likePost(userId: number, postId: number) {
    ...
    if (existingLike) {
      throw new ApiException('POST_ALREADY_LIKED', 409, 'E_POST_ALREADY_LIKED')
    }
  ...
  public async unlikePost(userId: number, postId: number) {
    const like = await Like.query().where('user_id', userId).where('post_id', postId).first()

    if (!like) {
      throw new ApiException('LIKE_NOT_FOUND', 404, 'E_LIKE_NOT_FOUND')
    }
  ...
  public async getLikesByPostId(postId: number) {
    return await Like.query().where('post_id', postId).preload('user').orderBy('created_at', 'desc')
  }
